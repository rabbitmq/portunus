...
the desired children from `init/1` once and registers each one. The
registry owns the local supervisor, so the whole tree shares one lifetime
and nothing is leaked or orphaned. Children are namespaced by group (the
